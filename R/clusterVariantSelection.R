...
clusterVariantSelection <- function(sce, variants.of.interest, n.clust, 
  ...
  if (!method %in% c('leiden', 'k-means', 'dbscan')) {
    stop("Invalid clustering method. Only 'leiden' and 'k-means' are valid for this parameter.")
  }
  ...
  if (method == 'k-means'){
    if (is.null(n.clust)) {
      stop("Parameter 'n.clust' must be provided when using 'k-means' clustering.")
    }
    ...
    if (!exists('min.pts'))  min.pts <- dim(df)[1]/100
